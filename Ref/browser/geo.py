...
AUTH = environ.get("AUTH", default="")
TARGET_URL = environ.get("TARGET_URL", default="https://geo.brdtest.com/mygeo.json")
...
def scrape(url=TARGET_URL, location=LOCATION):
    ...
    try:
        print("Connected! Changing proxy location" + f"to {location} ({lat}, {lon})...")
        driver.execute(
            "executeCdpCommand",
            {
                "cmd": "Proxy.setLocation",
                "params": {"lat": lat, "lon": lon, "distance": 50},  # kilometers
            },
        )

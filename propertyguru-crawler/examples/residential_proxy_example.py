...
from utils.proxy import StaticProxy  # noqa: E402
    ...
    for i in range(5):
        try:
            response = requests.get(
                "https://httpbin.org/ip", proxies=proxies, timeout=30, verify=False
            )
...
def example_2_batch_crawl_with_proxy():
    ...
    # 手动设置代理
    spider.session.proxies = {"http": proxy_url, "https": proxy_url}
    ...
    def parse_and_save(response):
        ...
        try:
            ...
            if response.url.endswith("/anything"):
                ...
                data = {"url": response.url, "status": response.status_code}

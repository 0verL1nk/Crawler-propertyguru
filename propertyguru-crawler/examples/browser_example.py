...
from crawler.browser import RemoteBrowser, scrape_with_browser  # noqa: E402
...
def example_1_basic_scraping():
    ...
    try:
        ...
        data = browser.execute_script("return arguments[0].map(el => el.innerText)", paragraphs)
    ...
    try:
        ...
        try:
            browser.download_file(
                download_trigger=lambda: browser.find_element(By.CSS_SELECTOR, selector).click(),
                filename=filename,
                timeout=60,
            )
...
def example_5_advanced_scraping():
    ...
    # 使用便捷函数
    result = scrape_with_browser("https://example.com", callback=parse_page)

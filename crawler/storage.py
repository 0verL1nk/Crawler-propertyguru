...
class S3Manager:
    ...
    def _create_bucket(self):
        ...
        try:
            ...
            logger.info(f"S3桶创建成功: {self.bucket_name}")
        except ClientError as e:
            logger.error(f"创建S3桶失败: {e}")
            raise
    ...
    def upload_file(self, local_path: Union[str, Path], s3_key: str = None,
        ...
        try:
            ...
            s3_key = self._get_full_key(s3_key)
            ...
            logger.info(f"文件上传成功: {local_path} -> s3://{self.bucket_name}/{s3_key}")
            return True
        except ClientError as e:
    ...
    def upload_fileobj(self, file_obj: BinaryIO, s3_key: str,
        ...
        try:
            ...
            logger.info(f"文件对象上传成功: s3://{self.bucket_name}/{s3_key}")
            return True
        except ClientError as e:
    ...
    def download_file(self, s3_key: str, local_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            logger.info(f"文件下载成功: s3://{self.bucket_name}/{s3_key} -> {local_path}")
            return True
        except ClientError as e:
    ...
    def download_fileobj(self, s3_key: str, file_obj: BinaryIO) -> bool:
        ...
        try:
            ...
            logger.info(f"文件对象下载成功: s3://{self.bucket_name}/{s3_key}")
            return True
        except ClientError as e:
    ...
    def copy_file(self, source_key: str, dest_key: str) -> bool:
        ...
        """
        try:
            source_key = self._get_full_key(source_key)
            dest_key = self._get_full_key(dest_key)

...
class Response:
    ...
    def __init__(self, url: str, status_code: int, content: bytes,
                 headers: dict, encoding: str = 'utf-8'):
        self.url = url
...
class Spider:
    ...
    async def async_fetch(self, session: aiohttp.ClientSession, url: str,
                         method: str = 'GET', **kwargs) -> Optional[Response]:
        """
        异步HTTP请求
    ...
    async def async_crawl(self, urls: List[str], callback: Callable = None):
        """
        异步批量爬取
    ...
    def crawl(self, urls: List[str], callback: Callable = None,
    ...
    def start(self, urls: List[str], callback: Callable = None, async_mode: bool = False):
        """
        开始爬取
    ...
    def save_to_db(self, data: Union[dict, List[dict]], collection: str = None):
        """
        保存数据到数据库
